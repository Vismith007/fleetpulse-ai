...
const TEMPLATES: LogTemplate[] = [
  { level: 'info',  message: (d) => `Telemetry packet transmitted. Seq=${Math.floor(Math.random() * 99999)}`,                          condition: () => true,                            weight: 40 },
  { level: 'debug', message: (d) => `Heartbeat ACK received. RTT=${Math.round(Math.random() * 12 + 1)}ms`,                             condition: () => true,                            weight: 20 },
  ...
  { level: 'error', message: (d) => `Packet loss detected on uplink channel. Retransmitting...`,                                        condition: (d) => d.signalStrength < -80,         weight: 5  },
  { level: 'error', message: (d) => `CPU watchdog timeout. Process restarted automatically.`,                                           condition: (d) => d.cpuUsage > 85,               weight: 4  },
  ...
  { level: 'debug', message: (d) => `Config sync completed. Hash=0x${Math.floor(Math.random() * 0xffffff).toString(16).toUpperCase()}`,condition: () => Math.random() < 0.15,            weight: 5  },
  ...
  { level: 'error', message: (d) => `Authentication token expired. Refreshing...`,                                                      condition: () => Math.random() < 0.02,            weight: 1  },
